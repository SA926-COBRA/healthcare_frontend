...
const Relatorios = () => {
  ...
  const loadReportData = async () => {
    try {
      ...
      setReportData({
        ...
        doctors: [] // Will be populated from appointments
      });
      
      // Calculate summary statistics
      const totalRevenue = financialData.reduce((sum: number, billing: any) => 
      ...
      const occupancyRate = appointmentsData.length > 0 ? 
        Math.round((todayAppointments / appointmentsData.length) * 100) : 0;
  ...
  const generateFinancialReport = () => {
    ...
    const csvContent = [
      ['Data', 'Paciente', 'Médico', 'Serviço', 'Valor', 'Status', 'Método de Pagamento', 'Convênio'],
      ...financialData.map((billing: any) => [
  ...
  const generatePatientsReport = () => {
    ...
    const csvContent = [
      ['Nome', 'CPF', 'Email', 'Telefone', 'Data de Nascimento', 'Gênero', 'Convênio', 'Status'],
  ...
  const generateAppointmentsReport = () => {
    ...
    const csvContent = [
      ['Data', 'Horário', 'Paciente', 'Médico', 'Status', 'Convênio', 'Observações'],
  ...
  const generateDoctorsReport = () => {
    ...
    const csvContent = [
      ['Médico', 'Total de Consultas', 'Pacientes Únicos'],
      ...Object.entries(doctorStats).map(([doctor, stats]: [string, any]) => [
        doctor,
        stats.appointments,
        stats.patients.size
      ])
          ...
          <Card>
            ...
            <CardContent>
              {loading ? (
                ...
                <div className="h-64">
                  <Bar data={consultationsData} options={chartOptions} />
                </div>
              )}
            </CardContent>
          </Card>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t">
                ...
                <Button 
                  ...
                  onClick={() => {
                    ...
                    loadReportData(); // Reload data with new filters
                  }}
                >
                  Aplicar Filtros
                </Button>
