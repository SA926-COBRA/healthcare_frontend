...
const Licencas = () => {
  ...
  const loadLicensesData = async () => {
    try {
      ...
      // Load licenses from database (simulated for now since we don't have licenses API)
      const mockLicenses = [
        {
          id: '1',
          type: 'CliniCore Pro',
      ...
      const totalUsers = mockLicenses.reduce((sum, l) => sum + l.users, 0)
      ...
      const totalRevenue = mockLicenses.reduce((sum, l) => sum + l.price, 0)
  ...
  const handleSaveLicense = async (licenseData: any) => {
    try {
      ...
      setSelectedLicense(null)
      await loadLicensesData() // Reload data
    } catch (error) {
  ...
  const handleRenewLicense = async (license: any) => {
    try {
      const updatedLicenses = licenses.map(l => 
        l.id === license.id 
          ? { ...l, expiryDate: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000), status: 'active' }
          : l
      )
      setLicenses(updatedLicenses)
  ...
  const handleExportLicenses = async () => {
    try {
      const csvContent = [
        ['ID', 'Tipo', 'Status', 'Usuários', 'Máx Usuários', 'Data Expiração', 'Preço', 'Ciclo Cobrança', 'Auto Renovação'],
              ...
              {licenseTypes.map((type) => (
                <div key={type.value} className="p-4 rounded-lg border border-border hover:border-primary/50 transition-colors">
                  ...
                  <div className="space-y-2">
                    ...
                    <Button className="w-full medical-button-primary mt-4">
                      Contratar
                    </Button>
                  </div>
                </div>
