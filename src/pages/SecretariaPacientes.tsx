...
const SecretariaPacientes = () => {
  ...
  const loadPatients = async () => {
    try {
      ...
      setPatients(patientsData.map((patient: any) => ({
        ...
        email: patient.email || '',
        phone: patient.phone || '',
        birthDate: patient.birth_date ? new Date(patient.birth_date).toLocaleDateString('pt-BR') : '',
        gender: patient.gender === 'male' ? 'Masculino' : patient.gender === 'female' ? 'Feminino' : 'Outro',
  ...
  const handleExportPatients = async () => {
    try {
      ...
      const csvContent = [
        ['Nome', 'CPF', 'Email', 'Telefone', 'Data de Nascimento', 'Gênero', 'Convênio', 'Status', 'Última Visita', 'Total de Consultas'],
  ...
  const handleSavePatient = async (patientData: any) => {
    try {
      if (editingPatient) {
        await apiService.updatePatient(editingPatient.id.toString(), patientData);
  ...
  const handleDeleteSelected = async () => {
    ...
    if (confirm(`Tem certeza que deseja excluir ${selectedPatients.length} paciente(s)?`)) {
      try {
        ...
        await Promise.all(
          selectedPatients.map(id => apiService.deletePatient(id.toString()))
        );
      ...
      <div className="grid gap-4 md:grid-cols-4 mt-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {patients.filter(p => {
                const patientDate = new Date(p.birthDate.split('/').reverse().join('-'));
