...
const Demo = () => {
  ...
  const loadDatabaseData = async () => {
    try {
      ...
      const [patientsData, appointmentsData, billingsData] = await Promise.all([
      ...
      setDbData({
        ...
        inventory: [], // Will be populated from inventory data
  ...
  const handleAddInventoryItem = async () => {
    try {
      ...
      // Simulate inventory API call (since we don't have inventory API yet)
  ...
  const handleExport = async () => {
    try {
      ...
      const csvContent = [
        ...
        ...currentData.map((item: any) => Object.values(item))
  ...
  const handleDelete = (item: any) => {
    switch (activeTab) {
      case 'patients': deletePatient(item.id); break
      case 'appointments': deleteAppointment(item.id); break
      case 'inventory': deleteInventoryItem(item.id); break
      case 'transactions': break // Transactions usually shouldn't be deleted
      case 'pdf-test': break // No action for PDF test
    }
  }
